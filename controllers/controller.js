...
exports.getFechasNot = (req, res) => {
  ...
  db.Empresas.findOne({
    where: { nombre: req.params.empresa },
    include: [{ model: db.Dias }]
  }).then(empresas => {
    console.log(empresas.Dias);
    const dias = empresas.Dias.map(function(x) {
      return { [Op.ne]: x.id };
    });
    console.log(dias);
    db.Fechas.findAll({
...
exports.getCitasFechaEmpresa = (req, res) => {
  // Traer todas las empresas
  db.Citas.findAll({
    where: { empresa: req.params.empresa, fecha: req.params.fecha }
  }).then(
    horarios => {
